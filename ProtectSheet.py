...
def lock_sheet(wb, ws, df, lock_type, initial_index, sheet_password, unlocked_row=None, currency='£', last_index_unlocked=1000):
    """
    ...
    if lock_type == 'headers_only':
        ...
        if unlocked_row is None:
            ...
            for col in df.columns:
                col_letter = xlsxwriter.utility.xl_col_to_name(col)
                ws.set_column_pixels(0, 0, width=width, cell_format=eval(unlocked_f))
                ws.unprotect_range(f'{col_letter}{initial_index+1}:{col_letter}{last_index_unlocked}')

    
    if lock_type == 'headers_rows':
